    ...
    @endphp
          ...
          <p class="card-text"><a href="{{ route('instructor.details',$course->instructor_id) }}">{{ $course['user']['name'] }}</a></p>
                ...
                <div class="d-flex justify-content-between align-items-center">
                    ...
                    <div class="shadow-sm cursor-pointer icon-element icon-element-sm" title="Add to Wishlist" onclick="addToWishList (this.id)" id="{{ $course->id }}"><i class="la la-heart-o"></i></div>
                </div>
            </div><!-- end card-body -->
        </div><!-- end card -->
    </div><!-- end col-lg-4 -->
...
<div class="tooltip_templates">
    ...
    @endphp
            ...
            <div class="d-flex justify-content-between align-items-center">
                ...
                <button type="submit" class="mr-3 btn theme-btn flex-grow-1" onclick="addToCart({{ $item->id }}, '{{ $item->course_name }}', '{{ $item->instructor_id }}', '{{ $item->course_name_slug }}')"><i class="mr-1 la la-shopping-cart fs-18"></i> Add to Cart</button>
